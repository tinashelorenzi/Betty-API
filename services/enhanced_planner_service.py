    ...
    async def get_planner_dashboard(self, user_id: str) -> PlannerDashboard:
        ...
        try:
            ...
            overdue_tasks = [
                ...
                if t.due_date and datetime.fromisoformat(t.due_date).date() < date.today()
            ]
            
            # Get upcoming tasks (next 7 days)
            upcoming_end = date.today() + timedelta(days=7)
            upcoming_tasks = await self.get_tasks(
                user_id, 
                completed=False,
                due_date_range=(date.today(), upcoming_end)
            )
            ...
            return PlannerDashboard(
                stats=stats,
                upcoming_tasks=upcoming_tasks[:5],  # Top 5 upcoming
                recent_notes=recent_notes,
                calendar_events=calendar_events[:5],  # Next 5 events
                overdue_tasks=overdue_tasks[:3]  # Top 3 overdue
            )
